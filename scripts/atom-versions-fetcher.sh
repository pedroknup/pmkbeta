...
CURRENT_ATOM_ELECTRON=$(grep ".*: .*"https://raw.githubusercontent.com/atom/atom/master/package.json | grep -iw electronVersion | cut -d: -f2)
CURRENT_ATOM_ELECTRON_VERSION="${OUTPUT//[!0-9.]/}"

echo ""
echo "Current Atom version is using Electron@${CURRENT_ATOM_ELECTRON_VERSION}"

rkamyp2_FOLDER="${PWD}/pmkbeta"

if [ -d ${rkamyp2_FOLDER} ]; then
  echo "pmkbeta repository found. Performing a pull..."
  cd pmkbeta
  git checkout master
  git pull
else
  echo "pmkbeta repository not found"
  git clone https://github.com/pycom/pmkbeta-atom.git pmkbeta
  cd pmkbeta
fi
...
if [ "$CURRENT_ELECTRON_VERSION" == "$ELECTRON_VERSION" ]; then
  RESULT="pmkbeta's Elektron is synchronized with Atom ${LATEST_VERSION_NUMBER}!"
else
  RESULT="pmkbeta's Elektron is NOT synchronized with Atom ${LATEST_VERSION_NUMBER}!"
fi

echo ""
echo "Elektron versions:"
echo "pmkbeta: ${CURRENT_ELECTRON_VERSION}"
...
cd .. 
ls
echo "Downloading binaries for ${ELECTRON_VERSION}..."
pwsh scripts/mp-download-atom.ps1  -ElectronVersions \"${ELECTRON_VERSION}\"
