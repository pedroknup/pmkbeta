...
import pmkbeta from '../lib/pmkbeta';
...
describe('pmkbeta', () => {
  ...
  beforeEach(() => {
    ...
    activationPromise = atom.packages.activatePackage('pmkbeta');
  });

  // order:
  // - Open pmkbeta, check if element exists
  ...
  describe('pmkbeta open', () => {
    it('opens the terminal', () => {
      // Before the activation event the view is not on the DOM, and no panel
      // has been created
      expect(workspaceElement.querySelector('.pmkbeta')).toExist();

      // // This is an activation event, triggering it will cause the package to be
      // // activated.
      // atom.commands.dispatch(workspaceElement, 'pmkbeta:toggle');
      ...
      runs(() => {
        ...
        let pycomSyncAtomElement = workspaceElement.querySelector('.pmkbeta');
    ...
    it('connects to the device', () => {
      ...
      waitsFor(function() {
         // workspaceElement = atom.views.getView(atom.workspace);
         var title_el = workspaceElement.querySelector('.pmkbeta-title')
         if(title_el){
      ...
      runs(() => {
        let connected = workspaceElement.querySelector('.pmkbeta-title').innerHTML.indexOf(" Connected") > -1;
        console.log(workspaceElement.querySelector('.pmkbeta-title').innerHTML)
    ...
    //
    //   expect(workspaceElement.querySelector('.pmkbeta')).not.toExist();
    //
