...
export default class pmkbeta extends EventEmitter {
  constructor(serializedState, view, settings, isDark) {
    ...
    atom.themes.onDidChangeActiveThemes(() => {
      _this.checkTheme();
      if (_this.isDark) {
        $('#pmkbeta').removeClass('light');
        $('.pmkbeta-status-bar').removeClass('light');
        $('.pmkbeta-logo').attr(
          'src',
        ...
        $('#pmkbeta').addClass('light');
        $('.pmkbeta-status-bar').addClass('light');
  ...
  setEventListeners() {
    ...
    this.settings.on('format_error', () => {
      // _this.terminal.writeln("JSON format error in pmkbeta.conf file") //TODO change to toast / error popup?
    ...
    this.view.on('open', () => {
      ...
      _this.autoConnect.start(connectedOnAddr => {
        if (!connectedOnAddr) {
