...
export default class PanelView extends EventEmitter {
  ...
  build() {
    const _this = this;
    this.element = $('#pmkbeta');
    this.elementOriginal = this.element[0];
    this.resizer = $('#pmkbeta-resizer');
    this.overlay_contents = $('#pmkbeta-overlay-contents');
    this.topbar = $('#pycom-top-bar');
    this.title = $('#pmkbeta-title');
    this.projects_button = $('#pmkbeta-projects');
    this.projects_list = $('#pmkbeta-projects .subnav');
    this.project_name_display = $('#pmkbeta-projects #project-name');
    this.project_name = '';
    this.buttons = $('#pmkbeta-buttons');
    this.button_more_tab = $('#pmkbeta-more-subnav');
    this.overlay_wrapper = $('#pmkbeta-overlay');
    this.terminal_area = $('#pmkbeta-terminal-area');
    this.terminal_placeholder = $('#pmkbeta-terminal-placeholder');
    this.button_more = $('#pmkbeta-buttons #more');
    this.button_more_sub = $('#pmkbeta-buttons #more .subnav');
    this.button_close = $('#pmkbeta-buttons #close');
    this.comport_list = $('#pmkbeta-comports-list');
    this.address_list = $('#pmkbeta-address-list');
    this.device_connection_tabs = $('#pmkbeta-connection-tabs');
    this.connect_all = $('#pmkbeta-connect-all');
    this.close_all = $('#pmkbeta-close-all');
    this.add_address_button = $('#pmkbeta-add-address-button');
    this.add_address_field = $('#pmkbeta-add-address-field');
    this.add_address_field_wrapper = $(
      '#pmkbeta-add-address-field-wrapper',
    );
  ...
  showFeedbackPopup() {
    ...
    if (!this.feedback_popup_seen) {
      ...
      this.feedback_question.classList.add('pmkbeta-feedback');
      this.feedback_question.innerHTML = '<h2>Hi pmkbeta User!</h2> ';
      this.feedback_question.innerHTML +=
        'We are working on ideas for pmkbeta 2.0 and would love your feedback! ';
  ...
  buildTerminal() {
    ...
    erd.listenTo(document.getElementById('pmkbeta'), () => {
      if (_this.visible) {
  ...
  initResize(resizer) {
    ...
    erdUltraFast.listenTo(
      document.getElementById('pmkbeta'),
      element => {
  ...
  createButton(id, icon, text, className, parent) {
    ...
    if (className && className !== '') {
      button.addClass(`pmkbeta-${className}`);
    }
  ...
  setProjectName(name) {
    ...
    $(document).ready(() => {
      const width = parseInt($('#pmkbeta-options-left').width());
  ...
  addComport(com_info) {
    ...
    $('#pmkbeta-comports-list .loading_text').remove();
  ...
  addAddress(address) {
    ...
    $('#pmkbeta-address-list .loading_text').remove();
    button.click(function() {
      _this.emit('connect.device', $(this).attr('name'));
    });
  }
  ...
  addConnectionTab(address) {
    ...
    $('#pmkbeta-terminal-placeholder').css(
      'font-size',
      `${fontSize}px`,
    );
  ...
  removeConnectionTab(address) {
    ...
    if ($('#pmkbeta-terminal-area div.device-terminal').length === 0) {
      this.terminal_placeholder.addClass('open');
      const runButton = $('#pmkbeta-action-connect');
  ...
  openInfoOverlay(info) {
    this.overlay_wrapper.addClass('pmkbeta-open');
    this.overlay_contents.addClass('pmkbeta-open');
    this.overlay.open(info);
    $('.xterm-rows').addClass('blur-text');
  }

  closeOverlay() {
    this.overlay_wrapper.removeClass('pmkbeta-open');
    this.overlay_contents.removeClass('pmkbeta-open');
