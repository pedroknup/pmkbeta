...
export default class InfoView extends EventEmitter {
  ...
  build(rootElement) {
    ...
    $(document).ready(function() {
      ...
      _this.info_content = $("#pmkbeta-info-view");
      _this.info_close = $("#pmkbeta-info-close");

      _this.info_close.click(function async() {
        try {
    ...
    $("button#info-button-free-memory").click(() => {
      ...
      commands.prepare(() => {
        commands.formatFlash(() => {
          _this.panelview.selectedDevice.commands.getFreeMemory(result => {
            commands.exit(() => {
