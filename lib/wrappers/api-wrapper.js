...
export default class ApiWrapper {
  constructor(settings) {
    atom.config.observe('pmkbeta.font_size', newValue => {
      try {
        ...
        atom.config.set(`pmkbeta.font_size`, 14);
      }
    });
  }

  config(key) {
    return atom.config.get(`pmkbeta.${key}`);
  }

  setConfig(key, value) {
    return atom.config.set(`pmkbeta.${key}`, value);
  }

  openSettings() {
    atom.workspace.open('atom://config/packages/pmkbeta');
  }
  ...
  onConfigChange(key, cb) {
    atom.config.onDidChange(`pmkbeta.${key}`, cb);
  }
  ...
  getPackagePath() {
    return atom.packages.resolvePackagePath('pmkbeta');
  }
  ...
  selectFirstProject() {
    if ($('.pmkbeta-project') && $('.pmkbeta-project').toArray()[0]) {
      $('.pmkbeta-project')
        .toArray()[0]
        .click();
      const projectsButton = $('#pmkbeta-projects');
  ...
  setFolder(name) {
    ...
    $('#pmkbeta-projects #project-name').html(
      `<span class='project-name'>${projectName}</span>`,
    );
    $(document).ready(() => {
      const width = parseInt($('#pmkbeta-options-left').width());
      $('#pmkbeta-connection-tabs').css('left', width + 5);
    });
  }
